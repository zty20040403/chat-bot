...
镜像加载服务不执行镜像删除。此前启动日志曾在 `Loaded image` 后立即出现当前镜像的
`Untagged` 和 `Deleted`，但服务仍报告成功；切换前的临时容器测试未覆盖这个收尾步骤。
现在启动必须通过普通沙盒用户的文件写入、Node、Nix 和中文 PDF 检查，并确认镜像标签
仍指向加载的镜像 ID，才能启动 Bot。自建镜像的运行命令禁止公网拉取；缺失时明确报告
本地部署问题。维护时的镜像回收应独立执行并排除当前配置镜像，不能放回加载流程。
