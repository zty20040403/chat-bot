...
收到的 **6 位验证码** 即完成授权，不用输入“确认”、操作编号或再点按钮。
当前任务和并行子任务共享授权，自动继续执行，不再逐条确认。
...
- 私聊直接回复 6 位数字，由服务器匹配本人的对应待办，不会默认批准最新一项或批准全部待办。
  极少数验证码碰撞时拒绝猜测，重发目标操作即可；旧的 `确认 AP-操作编号 6位口令` 格式仍兼容。
