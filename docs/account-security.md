...
管理员在网页或 QQ 发起需要审批的变更时，机器人把具体操作及完整参数私聊到该账户绑定的 QQ。本人回复 `确认 AP-操作编号 6位口令` 后自动执行，电脑无需保持页面打开。普通聊天回答、状态查询、固定目标诊断和下列隔离沙盒操作不要求口令；其他管理命令与配置更改仍需要口令。自动守护只能提出修复，每次实际修复仍须单独确认。
...
这些例外不包括宿主机命令、集群发布/资源租用、服务重启、网络和数据库变更或 rebuild。
