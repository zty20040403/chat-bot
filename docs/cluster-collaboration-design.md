# gaoji 八节点协作与远程运维完整方案：Ops 集成版

日期：2026-09-06。设计基线：gaoji 0.11.3，Bot 提交 `13accb2`，Nix 配置提交 `327cd3e`。

修订：第 2 版，明确复用 Ops，收窄自建节点程序职责。基线来自本次读取的本地源码和配置，不等于重新确认了所有生产节点的运行状态。

状态（2026-09-09 更新）：Bot 修复 `e175de0` 已通过共享 Nix 配置 `8e62c4d` 在 h310、tank、h610 串行部署验收，账号/OTP 并行修改未发布。三台均可观察、执行获批管理操作、领取 Worker 任务和提供限时预览。P5 队列堵塞已修复并通过三台线上复验；P6 已逐台验证停止 Worker 后的一次受限自动启动，恢复经验可检索且会检查适用条件。P7 三台精确版本构建与激活核验通过，控制服务重启后继续完成同一部署记录。数据库仍为 h610 主、tank 备，健康检查成功；Bot 已连接 QQ，群告警仍关闭。真实跨租约任务转移、QQ 文件交付和生产回滚演练尚未完成，不能用隔离测试或空闲 Worker 重启替代。逐项证据见 [三节点验收记录](three-host-rollout.md)；下文保留最初设计基线，不能当作实时运行状态。
...
| P1 只读接入 | 控制服务、数据库迁移、工具、指标和控制台完成 | 三台均返回经认证、未缓存的新鲜观察 | 保持时效标记，不能把一次可达说成永久健康 |
| P2 实验式排障 | 六类固定模板、结构化证据与两层检查完成 | 三台诊断任务 9/10/11 完成，各有 Worker 探测成功回执 | 六类故障的隔离测试不等于全部生产案例已验收 |
| P3 远程操作 | 合同、身份、审批、事件、幂等、租约与 Ops 管理账本完成 | 三台均已执行经批准的 `id -u` 验收 | 保持逐次审阅与准确主机范围 |
| P4 Worker/预览 | 独立轻量 Worker、资源预留、fencing、断线回执、产物哈希和静态预览完成 | 三台已部署，真实任务成功，健康端点 200、过期预览 404 | QQ 文件交付另行核验，不能用预览成功替代 |
| P5 借用调度 | 所有者策略、限时授权、资源预留与检查点已实现；七项隔离回归通过 | 三台线上超额任务不堵塞小任务、CPU/内存/GPU 拒绝原因明确，测试任务已取消 | 跨租约恢复目前只有隔离测试证据 |
| P6 故障记忆/守护 | 事故、案例检索、明确授权、原子次数预留和 Ops 修复回执完成 | 三台真实 Worker 均自动恢复一次；经验检索能区分已知原因与未知原因，临时授权已取消 | 尚不能把案例检索说成自动学习和自主决定修复 |
| P7 固定版本部署 | Ops 持久编排、两阶段审批、精确制品、串行验收与逆序回滚完成 | `8e62c4d` 三台构建、串行激活和双配置核验完成；实际只更新 h610 的两个 gaoji 服务 | 生产回滚未演练，保留隔离测试证据；不得回滚数据库修复或同学服务来凑验收 |

建议阅读顺序：第 1-5 节了解整体；第 6-10 节看五个功能怎么用；第 20 节看先做什么。第 11-19 节是实施时需要的接口、数据库、权限和失败处理细节。

一句话：**Ops 负责可信的服务器查询与其正式支持的受控运维接口，gaoji 负责理解目标、协作分析、授权编排、计算任务和结果交付；Prometheus、Nix、现有数据库各自继续做好原来的工作。**

本次相对上一版的调整：

| 原方案中可能重叠的部分 | 本版决定 |
| --- | --- |
| 自建 Node Agent 同时查状态、查日志和操作 systemd | 取消通用自建运维 Agent，先复用 Ops agent；确有缺口才做狭窄的执行扩展 |
| gaoji 再登记一套服务器和运维能力 | 主机标识源于 Nix registry；Ops 拥有其运维目录，gaoji 只保存投影及任务专属策略 |
| 一份本地注册表定义所有操作 | 各后端拥有自己的接口定义；gaoji 做版本化映射、收紧权限和任务验收，不复制上游定义 |
| 新的服务变更默认由 gaoji 自己执行 | 优先复用经确认的 Ops 写接口；当前未实现就不开放，独立扩展必须获准并明确唯一执行归属 |
| 监控和事件分别采集、分别通知 | 复用已有指标与告警来源；gaoji 负责调查关联，按事件去重通知 |
| 接入 Ops 就能管理全部机器 | 先添加 gaoji 独立只读身份，再逐台登记；算力节点不必同时开放运维权限 |
...
这些是相对于当前 gaoji 和参考部署的产品改进，不宣称是行业首创。
...
集群观测依赖外部运维 HTTP 操作目录；具体可用能力以部署配置和返回的目录版本为准。

- 当前只读适配器通过 Ops HTTP 操作目录查询，不把 SSH 密钥交给模型。
...
- 本地配置只登记 h610；客户端是 `max`、`hank`，尚无 `gaoji`。
- 可读服务必须由部署方逐项配置，以实际操作目录和服务白名单为准。
...
当前部署配置以 共享 Nix 仓库中 h610 的运维配置模块 为依据。不能因为 Ops 文档的未来示意图把 hub 放在 tank，就在 gaoji 中硬编码 tank 为当前 hub。

### 3.3 什么复用，什么自己做

| 能力 | 唯一职责来源 | gaoji 新增的部分 |
| --- | --- | --- |
| 主机稳定标识、架构和维护者基础资料 | 共享 Nix host registry | 引用 host_id，保存任务角色与临时借用策略 |
| Ops 支持的状态、服务与有限日志查询 | Ops hub / agent | 适配、会话授权、证据归纳和控制台展示 |
...
| 跨节点排障与协作任务 | gaoji 主控及独立上下文 Sub-Agent | 选检查、比较证据、分配工作、总结结果 |
| 沙盒、计算、产物和临时项目 | gaoji Worker 与项目发布执行器 | 隔离运行、资源借用、生命周期和交付 |
| 业务任务状态、操作合同与审批 | gaoji 控制服务 / PostgreSQL | 复用既有队列、审计、Outbox 和恢复语义 |

Ops 不需要读取 gaoji 的整段聊天、模型密钥或所有任务文件。gaoji 也不需要读取 Ops 的内部数据库、节点 token 或 其他用户 的凭据。双方通过正式 API 协作。

### 3.4 缺失的写操作怎么安排

采用“先确认能力，再开放工具”的顺序，而不是先放一个能执行任意 shell 的工具占位。

1. P1 只接 Ops 现有只读接口，P2 排障也可以先以只读证据工作。
2. P3 实施前与共享平台维护者确认服务变更的后端。如果 Ops 正式实现并通过授权、回执和状态查询验收，就接其写接口。
...
其他客户端、gaoji 和 CLI 若都能修改同一服务，互斥锁与去重必须落在它们共用的后端，不能只靠 gaoji 数据库里的锁。人工绕过后端操作仍可能发生，因此还要做状态前置校验和事后对账。
...
Ops 当前设计主要服务受管服务器。桌面、WSL、非支持系统不能为凑齐八台而强行安装相同 agent：由维护者明确支持范围。不支持运维接入的节点可只运行隔离 Worker，控制台显示“仅计算”；休眠或主人主动收回时按预期离线处理，不当成服务器故障反复告警。

资源登记分层：Nix registry 提供共同主机标识；Ops 配置提供它允许查询和操作的资源；gaoji 的借用策略只提供计算授权。三者以 host_id 关联，但任何一层都不能自动扩大另外两层权限。
...
flowchart TD
    ...
    HUB --> READ[Ops API 适配器]
    READ --> MH[现有 Ops Hub]
    MH --> MA[已登记的 Ops Agents]
...
| gaoji 控制服务 | 派工与记账的人 | 用户授权、排队、资源分配、保存记录、核对结果；不复制 Ops Hub |
| Ops hub / agent | 查询服务器的统一入口 | 管理其操作目录、主机与服务授权，提供实际支持的观测 |
| 受控动作 / 部署执行器 | 按批准内容修改资源的人 | 服务动作由唯一后端负责，部署由固定版本与范围的执行合同负责 |
| gaoji Worker | 干重活的执行器 | 在隔离工作区运行代码、构建、转码；回报自身任务和资源，不通用管理宿主服务 |

集群控制服务与 Bot 分开为 systemd 服务，首期都可在 h610。这样重启 Bot 不会把它发出的重启任务一起杀掉。两者分开不等于高可用：h610 整机不可用仍会影响控制服务。

首期复用 Python 的合同与存储实现。Ops agent 继续使用上游包，不另写 Python 替代版。需要 Worker 的节点才安装独立 Worker 包和对应运行时，不把整个 NoneBot 插件、浏览器和媒体依赖装到每台机器。狭窄动作扩展如果获准存在，也独立打包。
...
- 控制服务通过 Ops hub 查询服务器，不绕过 hub 直连其 agent；Worker 通过认证的领取接口取得分配给本机的任务。
...
QQ / 网页的真实用户请求
    ...
    -> 仅映射已支持版本、已批准的只读操作
    -> 校验目标与参数，必要时发起查询
    -> Ops 再检查客户端、主机、服务和能力授权
    -> 保留来源、观测时间、未知状态与分节点错误
    -> 按当前会话权限裁剪、脱敏
    -> 保存最少必要证据，返回给模型 / 控制台
```

gaoji 先按上游版本 1 的目录实现兼容。操作目录不是免审工具集，出现新操作不会自动对所有群开放。启动或重连时检查能力，调用前再次确认允许的操作；参数 schema 不兼容就停止该映射并报告，不能转换成 shell 兜底。

gaoji 适配器的传输约束为：4 KiB 请求、2 MiB 响应、单个 HTTP 请求最多 15 秒、不跟随重定向、不隐式使用环境代理、不反射秘密或原始错误正文。gaoji 可设置更短的整次查询预算，操作目录和执行共用剩余时间，不把两个 15 秒当成一次 15 秒承诺。

只读连接错误可在总预算内有限重试；401/403、schema 错误和能力不支持不重试。写操作不复用这种自动重试。地址由部署配置注入，模型不能提供 Ops URL、代理、token 或身份。

### 5.4 独立凭据与双层权限

新增专用 hub 客户端 `gaoji` 和 SOPS 凭据，例如 `ops/gaoji_token`；这是拟新增名称，不是已经存在的 secret。默认只授予批准主机上的查询能力，日志权限按需要单独开放。hub 的可读服务清单需由维护者显式加入 gaoji 和批准的依赖服务。

当前 hub 接入以服务客户端鉴权，不假设已经支持 QQ 用户代理或逐用户 token。因此权限分两层：gaoji 核对最终用户与会话；hub 核对 gaoji 这个服务身份的最大权限。这个服务身份被攻破后的影响范围等于其所有 grants，不能把应用内用户过滤当成 hub 的逐用户隔离。

日志等敏感能力应使用更窄的主机/服务授权；如果上游当前粒度不能满足隔离需求，就分离客户端及受限实例，或者不开放那部分能力。凭据只装入控制服务，不给 LLM、浏览器、任务沙盒和 Worker。其他客户端 的现有身份保持不变。

群聊输出按整个可见会话判断权限，不因提问人是管理员就把私有日志发给全群。需要更高权限的详情进入经过身份验证的控制台；不在群里生成绕过鉴权的公开下载链接。

gaoji 已获知的撤权会阻止后续调用、检索和新上下文注入，但不能撤回用户已经看到、下载或转发的内容；群权限必须在首次发送之前检查。上游撤权的发现时效另按第 5.5 节处理，不能假设两个进程之间存在尚未实现的即时通知。

### 5.5 状态缓存、查询负载与实时性

复用上游监控采样；gaoji 缓存的是查询投影，不是新的监控数据库。首期可按需缓存状态 15-30 秒、基本事实几分钟，具体时效按操作定义调整；日志默认按需读取，不全量轮询。

缓存按后端身份、本地授权版本、目标、操作和参数隔离。命中缓存仍要检查当前本地权限，已知撤权后不可继续用旧目录或旧缓存返回受限数据。并发相同查询可以合并；审批与服务变更前的状态检查必须满足合同要求的新鲜度，不能沿用普通页面缓存。

当前接口不假定提供上游授权版本或撤权事件。本地用户/群撤权立即阻止新返回；hub 单独改变 grants 时，gaoji 只能在下一次授权查询中获知，不能承诺零延迟同步。敏感日志不做跨请求结果缓存，必须重新通过上游执行鉴权；批准使用短期缓存的非敏感状态有明确的最多 30 秒刷新/失效窗口，遇到拒绝立即失效。若某类资料要求每次读取均以实时上游授权为准，就禁用它的跨请求缓存。已受理的读取可能完成，投递前仍检查本地会话权限。

现有 Ops 接口不假定有 SSE。后台适配器做有界轮询与按需查询，再把有变化的投影通过 gaoji 已有 SSE 发给网页。浏览器的实时更新不是每位访客各自向八台机器持续查询。

Ops 超时显示“运维数据暂不可用，最后更新于具体时间”，不是“所有服务器已关机”。失败时保留过期标记，不自动改走 SSH、复用他人凭据或安装第二套探针。普通聊天不等待 Ops；其查询使用独立并发与超时预算，故障不会耗尽聊天请求槽位。
...
这一流程先读 Ops 的现有证据；需要额外检查时再派发探测。主控保存整体调查，网络、模型和服务检查各有独立子上下文，只接收任务目标、必要证据及明确上游结果。它们不能继承管理员的全部权限，也不会把其他群的聊天带进排障。

### 6.2 哪些检查走 Ops

| 检查 | 首选来源 | 缺能力时怎么办 |
| --- | --- | --- |
| 已受管主机事实、允许服务状态与日志 | Ops 正式目录 | 显示未接入或无权限，不直接登录主机绕过 |
| 已有指标与活动告警 | Ops 聚合及现有监控接口 | 仅在明确配置了独立只读监控身份时使用该来源，并标明来源不同 |
...
| Bot 内部 Trace、模型阶段、Outbox 与数据库等待 | gaoji 已有可观测性 | 和主机证据关联，不要求 Ops 理解 Bot 的全部内部表 |
...
```json
{
  "evidence_id": "ev_001",
  "incident_id": "inc_001",
  "source_backend": "gaoji-fixed-probe",
...
Ops 返回的观测记录同样标注来源操作、后端请求引用（如果提供）、观测时间与接收时间。来源没有提供精确观测时间时显式标记未知，不拿接收时间冒充机器实际测量时间。模型解释单独保存，不能覆盖事实字段。
...
Ops 在这条链路中用于读取节点和允许服务的观测，不能把目前的只读 API 当作项目部署器。项目专属健康由发布执行器检查；不为每个临时预览任意扩大 Ops 的宿主服务白名单。
...
选择目标在 PostgreSQL 事务中预留槽位、内存和 GPU 资源，Worker 接收后再按本机实际状态核对。资源不够就拒绝接单并返回原因，不在机器上硬启动。多个 gaoji 调度进程争抢同一资源时由数据库约束防止重复预留；它不能阻止同学在别的系统中启动负载，因此仍需本机配额、接单复核和容量余量。

Ops 的空闲内存或 GPU 指标是观察，不是资源预留许可。实际可借额度由主人策略与 Worker 上报共同确定。共用同一 GPU 的其他调度器如果没有共同预留协议，就按独占借用时段或独立配额运行，不承诺跨系统精确超分控制。
...
活动快照不能还原离线期间所有告警次数。保留现有告警事件接收与历史记录为生命周期来源，Ops 的 `alerts.active` 用来补充当前观测和交叉核对。按来源、稳定告警标识、发生周期和状态版本去重；相同告警恢复后再次触发仍算新一轮。

首期不增加另一条相同告警的群推送链路。将来 Ops 启用通知后，由维护者约定同一事件的主要发送方；gaoji 只补充独立调查和处理阶段，不和 其他客户端 同时重复播报相同告警。对方无法协同去重时，gaoji 的相关自动报告可关闭，人工查询仍可用。
...
当前仅完成 Ops 只读接入时，守护只能“发现并通知”。只有相应资源具备已验收的写后端和生效授权后，才显示“可自动修复”。数据过期、Ops 不可达或只是观察器故障时，不能凭旧状态触发重启。
...
| 入口 | 适用范围 | 执行方式 |
| --- | --- | --- |
| 结构化运维操作 | 查服务、重启、部署、注册预览 | 查询走 Ops；变更按唯一后端映射，固定参数、已登记资源、明确验收 |
...
后端受理后另存 `backend_operation_id`（若支持），把任务、批准、上游审计与节点回执关联。不存在上游字段时记录“不提供”，不自行伪造 Ops 的审计记录。写后端若不支持足够的结果查询和重复请求处理，不上线自动写流程。
...
Ops 查询继续使用其上游身份与资源限制。后续服务操作后端以专用身份运行，查询和特权动作分开授权。服务操作通过 systemd D-Bus 和精确到动作及 unit 的本地规则实现；可控制的服务定义与文件也必须不能被执行用户随意改写，否则会形成间接提权路径。这里是写后端上线要求，不宣称当前 Ops 已具备这些写能力。

当前 Ops 只读 HTTP 使用部署批准的 Tailnet 地址与独立 bearer 凭据，保持现有网络限制。新建写入与 Worker 管理链路要求 TLS 双向认证与短期执行授权；共享后端需满足约定的同等身份和抗重放要求才能开放写操作。密钥由 SOPS 或 systemd credentials 提供，不要求通过“加个开关”让当前只读服务突然支持另一套协议。网络可达范围与应用身份不能互相替代。
...
后端收到同一 operation_id 时返回已有状态；开始动作前持久记录接受的合同和执行代次。调用 systemd 后保存回执，并检查新的服务 invocation、状态与业务探针。HTTP 超时不会让控制服务立即创建另一个重启。Ops 写接口如果被采用，必须通过同样的契约验收，不能只因 HTTP 返回 200 就满足要求。

如果节点恰好在调用完成、回执落盘之前崩溃，恢复后必须查实际状态和可用证据，无法确认就保留未知。幂等键不能神奇保证所有外部动作恰好执行一次。

### 12.3 租约与 fencing

复用现有 durable job 租约模型；新增远程资源占用必须有数据库内原子分配与单调增长的执行代次。只读 Ops 不需要引入这一套执行租约；后续共享写后端负责所有写客户端的资源互斥和执行代次，gaoji 不能在本地自行声称已经锁住 其他客户端 的操作。
...
| 情况 | 行为 |
| --- | --- |
| Ops hub 不可达 | 运维观测标记不可用或过期；普通聊天继续，不判定全体主机故障，不改走 SSH |
| Ops 返回 401/403 | 报告凭据/授权问题并停止该查询；不换用 其他客户端 token |
| Ops 目录变更或操作消失 | 禁用不兼容映射、提示管理员；其他已确认能力继续运行 |
| 某个 Ops agent 不可达 | 仅该节点观测受影响，保留其他节点结果；区分 agent 故障与主机故障 |
...
| 其他客户端 或管理员同时变更同一服务 | 共享后端串行受理或明确冲突，执行前后校验版本；不由两个 Bot 各自判定成功 |
...
共享仓库发现同学的新配置时，先展示变化与影响，按照已有明确授权处理。不能强制覆盖脏工作树，也不能自动把“只更新 gaoji”扩大为任意其他服务变更。

跨机器升级先在一台合适的节点验收，再按依赖顺序继续；“暂停后续部署”和“回滚本次已部署节点”是两种不同动作，必须由合同说明。

部署可能修改多个服务，不能只拿一个 gaoji 本地“部署锁”就与 Ops 的服务重启并发执行。写接口上线前需约定共同的主机维护窗口与资源锁协议：部署申请受影响范围，服务动作后端检查该范围。暂不具备跨后端协调时，只允许串行人工安排的部署，不开放同时自动修复与系统切换。人工直接运行命令仍属外部变更，需要检测与报告，不能承诺技术上完全阻止。

### 13.2 复用 Nix 与 deploy-rs

复用仓库已有 flake、主机元数据和 deploy-rs。部署任务通过独立执行服务完成，不能放在即将被重启的 Bot 进程里。

Ops 提供部署前后允许读取的观测；其当前只读接口不承担 `nixos-rebuild switch`。不要把部署脚本塞进名为查询的 Ops 操作。主机范围、源提交与 closure 必须传给经批准的部署执行器，不能提供任意命令字符串。
...
1. Bot 仓库新增适配器、只读开关、超时与会话策略，不复制上游 Ops agent。
2. 共享 nix-config 新增独立 `gaoji` hub 客户端及其加密凭据，只追加获准主机和必要可读服务，不扩大原有 其他客户端 grants。
...
退出接入时先关闭 gaoji 查询和自动调查入口、撤销其客户端凭据并清除其受限缓存，再移除专属配置。不得停止其他客户端仍在使用的 Ops hub/agent，也不删除共享监控数据。
...
| 工具 | 作用 | 执行来源 |
| --- | --- | --- |
| `fleet_overview` | 查询当前用户可见节点、状态与观测时间 | Ops `fleet.overview` 与获准 Worker 投影，分别标记来源 |
| `host_inspect` | 查询批准字段，例如系统事实和服务状态 | Ops `host.facts` / `units.status`；缺字段返回未提供 |
| `service_logs` | 按允许服务和时间窗读取有限日志 | Ops `units.logs`，范围不得宽于用户与上游授权 |
| 现有 `query_alerts` | 查询当前告警及本 Bot 已收集的历史 | 复用现有历史；Ops `alerts.active` 补当前快照，不新增混淆语义的历史工具 |
| `probe_run` | 从批准观察节点执行固定类型探测 | 已登记探测后端或 Worker 模板，不接受任意命令 |
| `incident_open` / `incident_read` | 建立或读取带证据的调查 | gaoji 事件与证据存储 |
| `operation_prepare` | 提交远程动作建议，返回合同、影响和授权状态 | gaoji 检查唯一后端与能力，缺后端不创建可执行承诺 |
| `operation_status` / `operation_cancel` | 查看结果、取消尚可取消的工作 | gaoji 状态与原后端回执对账 |
| `cluster_job_submit` | 为任务步骤申请远程计算 | gaoji 队列、预留与 Worker |
| `preview_create` / `preview_extend` / `preview_promote` | 管理临时项目及正式化请求 | 专属项目发布与 Nix 部署执行器 |
| `guard_create` / `guard_pause` | 创建或暂停已定义的守护合同 | gaoji 状态机，观测与修复按各自后端授权 |
| `incident_search` | 在授权范围检索已验证故障经验 | 现有 BGE-M3 / PostgreSQL 检索体系 |

上述 Ops 名称是依据当前文档的初始映射，实际参数由认证目录 schema 校验，不在机器人里猜字段。第一期采用这些窄工具，不给模型一个能自由填写任意上游操作名的通用透传入口。控制服务内部仍通过统一 Ops 客户端调用。

有效能力取交集：真实用户授权、当前会话可见范围、专家角色授权、gaoji 适配器允许项、上游服务身份 grants，以及执行后端本地限制。缺少任一条件都不执行。普通聊天只装载需要的工具，不把全部集群 schema 塞进每轮上下文。
...
上游操作定义由 Ops 自己维护。gaoji 保存一份版本化映射，引用后端操作和 schema 摘要，再叠加更严格的范围、超时、风险和验收要求；不能修改上游 `read_only` 或扩展上游 grants。自有计算、预览和守护合同由本项目定义。

LLM 工具说明、API 校验和控制台能力清单从同一份已验证映射生成。上游 schema 变更后先验证兼容性，不在运行中静默接受新写字段。用于批准的操作版本还要包含后端映射版本，避免批准后切换执行目标。

### 14.3 不同后端的协议

Ops 使用现有 HTTP 目录与执行协议，不要求上游 agent 实现 gaoji 的队列、SSE 或工作区协议。Worker、固定探测执行器及获准的狭窄动作扩展握手报告自己的协议版本、boot_id、已实现操作、运行时和资源上限；这些声明只用于能力协商，不能扩大中央授权。

每个事件带 operation_id、执行代次与单调序号。API 不兼容时拒绝接单并显示原因，不能尝试用字符串命令兜底。滚动升级期间允许的兼容范围通过合同版本明确规定。

### 14.4 返回值不再只写“工具失败”

适配器把失败整理为明确状态，例如 `not_configured`、`unsupported`、`forbidden`、`unavailable`、`stale`、`invalid_request`、`conflict`、`outcome_unknown`。这些是 gaoji 拟定义的状态，不是宣称上游已经返回这些字段。
...
不复制 Ops 数据库、Prometheus 全量指标或全部 journal。保存选入本次调查的最少必要证据，记录来源和可见范围；日志证据可设短保留期，摘要和哈希可长期保留。上游未提供稳定证据句柄时，存本地引用并标明它是本次采集的副本。

Ops 操作审计是其后端事实，gaoji 审计是用户请求、计划、批准与任务事实，按请求/操作编号关联而不是互相冒充。删除或撤回访问权限后，摘要、embedding 命中和旧会话结果都不能成为继续读取原受限日志的旁路。
...
- 数据来源：Ops、现有监控和 Worker 的连接状态、覆盖范围、最近成功时间及禁用原因；不展示凭据。
...
同一页面分开显示“机器状态”和“数据来源状态”。例如 Ops hub 超时显示一个查询入口故障，不生成八条断电结论。只有 Worker 的桌面节点标记“仅计算”；未接入、无权限和预期离线分别显示。
...
Worker 心跳可每 15 秒左右一次；Ops 只读状态按第 5.5 节统一缓存与采样，不假定上游主动推送。资源趋势复用监控，操作进度在事件到达时更新。网页时钟每秒走动只使用浏览器本地定时器，不每秒请求八台机器。
...
第一版明确采用单控制服务，避免在新功能期间同时重做数据库 HA。h610 不可用时，Bot、同机控制服务以及当前 Ops hub / 监控入口都可能停止；tank 有数据不代表群聊或运维观测自动恢复。Ops 与 gaoji 进程独立不等于它们处在不同故障域。

在另一故障域布置轻量观察程序，独立检查 h610 和控制接口。检测目标从批准的资产配置中生成，告警不依赖 gaoji 自己判断自己是否在线。只有配置了真正独立可用的通知渠道，才能承诺 h610 掉线仍有通知。

优先复用真正独立部署的现有探针和通知设施；若只有 h610 上的 Prometheus，就不能把它称为独立观察。另一台机器如果共用断掉的路由、电源或唯一模型代理，也不是对应风险下的完整冗余。此阶段不擅自迁移共享 Ops hub 到 tank，更不把改变数据库主备当成接入前提。
...
src/cluster_control/              独立运行，避免与 NoneBot 初始化耦合
  ...
  guards.py                      有期限的守护状态机
src/cluster_worker/               自建执行能力，不复制 Ops agent
...
共享 nix-config                 复用上游 Ops module，注册客户端及获准节点
```
...
拟新增 `services.gaoji-control` 与 `services.gaoji-worker`，具体选项在实施时定义；不新增通用 `services.gaoji-node` 运维副本。外部运维服务沿用部署方现有模块和服务名，不随 gaoji 改名。Token、私钥和凭据不写 Git 明文或 Nix store。
...
| 阶段 | 用户能实际用什么 | 是否需要扩展共享 Ops |
| --- | --- | --- |
| P0 | 明确八台机器是谁的、允许做什么 | 确认配置与身份，不先操作机器 |
| P1 | 问服务器状态、有限日志，在控制台看全集群 | 独立客户端与逐台只读接入；不改 其他客户端 Bot |
| P2 | 自动跨节点查原因，给出证据 | 复用查询；缺的固定探测另行授权 |
| P3 | 获批重启指定服务并验证恢复 | 写后端能力须确认；无实现则保持只读 |
| P4 | 把任务交给远程 Worker，生成临时项目并交付 | 不依赖 Ops 变成计算平台 |
| P5 | 借用空闲资源、到期让路、分段恢复 | 复用观测，调度由 gaoji 负责 |
| P6 | 查相似故障、按期限守护服务 | 修复只使用已验收的写后端 |
| P7 | 固定版本部署、分批验收、受限恢复 | 当前使用 Ops 工作区、构建和部署协议作为唯一写后端 |

### P0：确认资产与底座合同

完成八节点名单、维护者、系统、角色、允许服务、网络入口、站点和资源清单；区分 observe/operate/compute，确认 Ops 支持与不支持的节点；定义操作与证据合同；完成网页及 QQ 操作者身份绑定设计。

验收：八台有唯一身份；未登记或未授权资源不进入可调用清单；同学的机器权限有明确来源。所有后续阶段引用同一 host_id；只读、写操作和算力借用分别批准。

### P1：全集群只读接入

实现独立控制服务、Ops 目录适配、权限交集、查询投影和控制台总览与详情。配置专用 hub 客户端，先接 h610 和获准的 Bot 服务，再逐台接入其他支持的批准节点；其他节点可以清楚显示尚未接入或仅计算。

验收：能回答“哪些机器异常、数据是什么时候取得的”；失联和过期不显示为健康；hub 故障不会误报八台全挂；本地撤权后不返回旧缓存，上游权限变化按约定窗口生效且敏感读取重新鉴权；故障清单与历史告警次数口径一致；下拉框不会被实时更新关闭。其他客户端 原有身份与服务行为不受影响。

### P2：实验式排障

接入六类优先排障模板、结构化证据、最多两层调查步骤、跨节点探测和运维专家结果综合。读取 Ops 与已有 Bot Trace，缺少的网络检查只提供最小固定探测执行配置，不提前建设完整 Worker 集群。每个检查任务独立上下文，say 对应真实阶段。
...
验收：重复受理返回同一操作记录；收到超时会先对账而不是再次重启；Bot 重启后仍能查询操作；修改目标或后端映射使旧批准失效；正常恢复判定包含业务探针；其他客户端/CLI 并发请求由共享后端协调。不能证明结果时如实保留未知。
...
`0.17.3` 实现说明：Worker 心跳建立所有者策略；缺少所有者策略时调度拒绝领取。跨所有者任务由服务端依据任务与 Worker 所有者身份强制要求限时授权，不能依赖模型自行声明。调度领取与资源、GPU、期限和最大费用预留在同一数据库事务内完成；授权资源按全部活动任务累计，预计费用只用于估算，实际回执超过硬上限会判失败。安全重跑与检查点资格由服务端任务目录固定，模型不能自行扩大；租约过期只重派目录标记为安全或拥有已完成检查点的任务，旧 fence 不能覆盖新结果。群文件交付另外使用不可变宿主快照和 QQ 回执核对，未确认时保留工作沙盒。
...
`0.17.3` 实现说明：排障失败与固定探针进入同一事故生命周期；管理员发布的已验证案例必须包含原因、适用条件、证据和明确置信度，随后进入 BGE-M3/pgvector 索引，搜索结果仍由控制服务根据当前主机和服务事实复核。Guardian 只访问登记目标，主机与服务由服务端绑定，并保存创建时的目标快照；登记目标发生变化时旧合同转人工确认。健康路径不调用 LLM；跳转页不算健康；需要人工关注时仍继续探测，恢复后自动关闭对应事故，到期后可靠结束。修复动作保存为不含运行时字段的固定模板，触发时才生成短期截止时间和稳定幂等键；只有写后端与资源授权当前均可用时才创建并消费守护预授权，不可执行状态不会留下阻塞后续恢复的旧操作。处理未真正入队时转人工关注，避免重复操作。P3 写后端未获批前，控制 API 和 UI 都拒绝创建自动修复守护，只允许观察模式。

验收：旧案例会再次验证适用条件；正常巡检不消耗 LLM Token；处理次数达到授权上限就停止；守护与借用到期行为可靠。

### P7：部署协作与持续完善

接入固定提交部署、变更预览、逐节点验收和受约束回滚，与服务操作后端约定维护协调。等基础恢复与审计稳定后，再评估完整控制服务高可用以及更多平台入口；不把这一步默认扩大为迁移共享 Ops 或重做数据库集群。
...
以上 `0.17.0` 描述的是早期 SSH 部署器。`0.19.0` 起，h310、h610、tank 的生产配置改为
`backend = ops`：gaoji 保存两阶段合同、批准和各目标进度；Ops 管理独立工作区、构建、
激活、验收与条件回滚，不再同时部署第二套 SSH 写入器。旧 SSH 后端仅留作显式配置的兼容选项。

预检校验完整提交、远端分支、工作区状态、锁文件和当前运行/持久系统路径。批准后仍重新校验，
前一节点验收成功才继续下一节点；控制服务重启后接管原操作账本，不新建重复写操作。
写操作发出前的只读检查允许有限重试；已发出但回执丢失的动作先对账，无法证明时停止。
维护窗口阻止守护与部署并发修改同一主机。没有故意在生产数据库或业务服务上制造回滚故障；
部分失败、受限回滚和恢复场景使用独立 PostgreSQL schema 与模拟 Ops 验证。
...
| 场景 | 必须观察到的结果 |
| --- | --- |
| Ops hub 超时，其他服务仍正常 | 只报告观测入口不可用，聊天不被拖慢，不产生八条主机故障 |
| 刚读取日志后本地用户被撤权，再请求相同内容 | 缓存、检索与新上下文均不返回受限日志 |
| hub 单独撤回 gaoji 的授权 | 新敏感读取被拒绝，其他授权视图在约定窗口内失效，不假称瞬时同步 |
| 上游只读目录新增未知写操作或 schema 不兼容 | 本地不自动开放工具，不改走任意命令 |
| 节点只有 compute 权限，用户要求重启宿主 | 不执行宿主操作，允许的任务能力不受影响 |
| 其他客户端 和 gaoji 同时要求修改同一服务 | 公共后端串行或明确冲突，不靠 Bot 各自的本地锁 |
...
- Ops 调用量、延迟、来源过期时长、权限拒绝和兼容失败；与普通聊天延迟分别统计。
...
2. 可单独部署的 gaoji 控制服务、Ops 适配器，以及专用只读客户端配置。
3. 集群只读工具、联合排障、必要的固定探测、证据记录和控制台详情；复用已有 Ops agents。
4. 经维护者确认后，指定服务操作的准备、授权、唯一后端执行、对账与验收闭环。
5. 针对关键故障的回归案例、部署说明和可操作的撤回办法。

随后交付远程 Worker、临时项目、算力借用、事件记忆和目标守护。不重新交付一套 Ops，也不把本方案当成 Ops 现有功能清单。

源码参考：

- [gaoji 基线](https://github.com/zty20040403/gaojibot/tree/13accb25105a207751f108c68b89e7b7d450f7c3)：现有任务、模型网关、权限和控制台。
- [共享 Nix 配置](https://github.com/imdomestic/nix-config/tree/327cd3e)：主机登记、监控、Ops 试点和声明式部署；文档里的长期规划与当前启用项要分别看。
...
最终分工不变：**Ops 把服务器情况查可靠；gaoji 把目标理解、任务协作、结果验收和交付做完整。**
