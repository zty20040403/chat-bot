...
状态：分阶段实施中。`0.15.0` 已完成 P0-P2，并实现 P3 的持久操作合同底座与 P4 的首个隔离 Worker。P3 在生产仍保持不可执行：MaxOps 当前只读，且尚未批准唯一写后端，因此控制台会明确显示 `not_configured`，不会用 SSH 或任意 root shell 冒充。P4 首批支持固定 HTTP 探测、产物校验、PDF/媒体检查和静态预览；是否上线以共享 Nix 配置和真实 Worker 心跳为准。P5-P7 尚未实现，不得把设计条目当作现有能力。
...
| P5-P7 | 仅保留设计 | 未部署 | 严格按本章阶段和验收逐期实现 |
