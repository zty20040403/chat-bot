...
状态：分阶段实施中。`0.12.0` 已完成 P0/P1 的 Bot 仓库实现，包括强类型资产合同、独立只读控制服务、MaxOps 目录适配、双层权限、持久查询投影、Tool Call、Prometheus 指标和 React 控制台。共享 Nix 身份、八节点生产清单和实际部署必须在同步共享仓库并完成验收后才算上线。P2-P7 尚未实现，不得把设计条目当作现有能力。
...
| P2-P7 | 仅保留设计 | 未部署 | 严格按本章阶段和验收逐期实现 |
