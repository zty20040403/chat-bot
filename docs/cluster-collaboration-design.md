...
状态（2026-09-07 更新）：分阶段实施中。h310、h610、tank 的 P3 已通过专用 Ops 管理身份完成审批和无害命令验收；不是任意群成员都有 root 权限。h610 已有 Worker，新增 h310/tank Worker 的加密凭据发布仍待授权。P6 受限修复和 P7 持久部署编排已接到同一 Ops 操作账本，并通过隔离 PostgreSQL 验证，但还未部署。P7 上游仍需支持干净工作区和指定旧提交，不能用制造新提交或另一套 SSH 写后端代替。逐项生产证据与未完成事项见 [三节点验收记录](three-host-rollout.md)；下文仍保留最初方案的架构基线，不代表所有阶段已上线。
...
| P1 只读接入 | 控制服务、数据库迁移、工具、指标和控制台完成 | h610 控制面已接入 Ops | 当前目标为三节点，最终验收需刷新三台实时观察证据 |
...
| P4 Worker/预览 | 独立轻量 Worker、资源预留、fencing、断线回执、产物哈希和静态预览完成 | h610 已使用；h310/tank 待部署 | 加密凭据发布授权、三台真实任务与到期预览验收 |
| P5 借用调度 | 所有者策略、available/draining/unavailable、限时授权、预算预留/结算、优先级与检查点恢复完成 | 待随 0.16.0 部署验收 | 用真实 Worker 验证撤权、超分拒绝、GPU 拒绝和跨租约恢复 |
| P6 故障记忆/守护 | 事故、案例检索、明确授权、原子次数预留和 Ops 修复回执完成 | 新版受限修复未部署 | 用隔离测试服务验证三台故障、恢复和历史经验，不干扰共享业务 |
| P7 固定版本部署 | Ops 持久编排、两阶段审批、精确制品、串行验收与逆序回滚完成 | 新编排未部署；上游仍有精确源码兼容缺口 | 补上游接口后验证三台真实预检、批准、取消和回滚；不同时开放 SSH 部署器 |
