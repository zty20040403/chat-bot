...
状态（2026-09-09 更新）：分阶段实施中。h310、h610、tank 均已接通只读观察、经批准的无害管理操作和独立 Worker；三台都成功执行了探测和限时预览任务。P5 线上资源验收发现超额任务堵塞后续小任务，本地已修复并通过七项隔离 PostgreSQL 回归，尚未发布。P6 的受限自动修复和历史经验复用仍未完成生产验收。P7 已通过同一 Ops 后端完成三台精确源码构建，并获批将 h610、tank 串行更新到 `82997e4`，运行和启动配置均已核验；h310 激活和回滚仍未验收。数据库现已恢复 h610 主、tank 备，两边健康检查连续成功，相关告警消失；Bot 运行中，群告警仍关闭。逐项证据与未完成事项见 [三节点验收记录](three-host-rollout.md)；下文保留最初设计基线，不能当作实时运行状态。
...
| P5 借用调度 | 所有者策略、限时授权、资源预留与检查点已实现；新队列修复本地七项回归通过 | 三台预览与撤权已验；本轮发现超额任务堵塞队列，修复尚未发布 | 发布后复验资源调度；跨租约恢复目前只有隔离测试证据 |
| P6 故障记忆/守护 | 事故、案例检索、明确授权、原子次数预留和 Ops 修复回执完成 | 三台观察守护到期完成，已有已验证案例 | 受限修复、恢复及后续历史经验复用仍需生产验收，不干扰共享业务 |
| P7 固定版本部署 | Ops 持久编排、两阶段审批、精确制品、串行验收与逆序回滚完成 | 三台精确源码构建已通过；h610、tank 的 `82997e4` 串行激活及双配置回执已验证 | h310 激活与回滚未验收；旧预检已结束，不得沿用旧审批 |
